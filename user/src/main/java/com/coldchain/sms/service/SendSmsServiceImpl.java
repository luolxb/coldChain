package com.coldchain.sms.service;

import com.alibaba.fastjson.JSONObject;
import com.coldchain.sms.entity.SendSmsEntity;
...
public class SendSmsServiceImpl {
    ...
    public String getCode(String mobile) {
        ...
        if (!NtsUtil.isPhone(mobile)) {
            throw new ShopException(-1,"手机号不正确");
        }
        // 删除之前的验证码
//        redisUtils.del(RedisKey.SMS_SEND + mobile);
        ...
        // 发送
        ResponseEntity<String> stringResponseEntity = new RestTemplate().postForEntity(sendUrl, sendSmsEntity, String.class);
        String body = stringResponseEntity.getBody();
        if (null == body) {
            log.error("获取验证码异常=>{}", stringResponseEntity);
            throw new ShopException(-1,"获取验证码失败");
        }
