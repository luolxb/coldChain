...
public class WxPayServiceImpl implements WxPayService {
    ...
    @Override
    public SBApi unifiedOrder(String orderNo, BigDecimal price, String trade_type, String body, String code, String callbackUrl) {
        ...
        try {
            ...
            // 总金额
            requestMap.put("total_fee", new BigDecimal(String.valueOf(price.multiply(new BigDecimal("100")))).stripTrailingZeros().toPlainString());
            // 终端IP
            requestMap.put("spbill_create_ip", new IpUtil().getIp());
...
//            requestMap.put("scene_info", "{\"h5_info\":{\"type\":\"Wap\",\"wap_url\":\"可以填应该网站的URL地址\",\"wap_name\": \"订单支付\"}}");
            // 接收微信支付异步通知回调地址
            requestMap.put("notify_url", ntsWxPayConfig.getPayNotifyUrl() + callbackUrl);
            requestMap.put("sign", WXPayUtil.generateSignature(returnMap, ntsWxPayConfig.getKey()));
            // 调用wxpay
...
//                        xmlBack = "<xml><return_code><![CDATA[SUCCESS]]></return_code><return_msg><![CDATA[OK]]></return_msg></xml>";
    ...
    public SBApi refund(String orderNo, double amount, String refundReason) {
        ...
        if (StringUtils.isBlank(orderNo)) {
            throw new ShopException(-1, "订单编号不能为空");
        }
        if (amount <= 0) {
            throw new ShopException(-1, "退款金额必须大于0");
        }
        ...
        if ("SUCCESS".equals(return_code)) {
            ...
            if ("SUCCESS".equals(result_code)) {
                ...
                //
                return new SBApi("退款申请成功");
            } else {
                log.info("订单号:{}错误信息:{}", orderNo, err_code_des);
                return new SBApi(-1, "失败", err_code_des);
            }
        } else {
            log.info("订单号:{}错误信息:{}", orderNo, return_msg);
            return new SBApi(-1, "失败", return_msg);
        }
    }
